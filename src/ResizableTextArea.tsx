...
class ResizableTextArea extends React.Component<TextAreaProps, TextAreaState> {
   ...
   * 字数限制文本
   */
  renderLimitText({ currentLength, maxLength, allowInputOverMax, prefixCls }) {
    const currentCls = classNames(`${prefixCls}-limit-current`, {
      [`${prefixCls}-limit-current-warning`]:
        allowInputOverMax && !!(maxLength && currentLength > maxLength),
    });
    return (
      <span className={`${prefixCls}-limit`}>
      ...
      <>
        ...
        {maxLength &&
          this.renderLimitText({
            ...
            maxLength,
            allowInputOverMax,
            currentLength: otherProps.value?.length,
          })}
      </>
    );
  };
